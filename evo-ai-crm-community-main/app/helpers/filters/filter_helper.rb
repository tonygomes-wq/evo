module Filters::FilterHelper
  ...
  def default_filter(query_hash, filter_operator_value)
    # Special handling for channel_type which is in the inboxes table
    if query_hash[:attribute_key] == 'channel_type' && filter_config[:table_name] == 'conversations'
      "inboxes.channel_type #{filter_operator_value} #{query_hash[:query_operator]}"
    else
